    ...
    else if(in->topicType == "interfaces/msg/CartData")
    {
        auto cb = [=](const interfaces::msg::CartData::SharedPtr msg) { ros_callback__interfaces__msg__CartData(msg, subscriptionProxy); };
        rclcpp::QoS qos = 10;
        subscriptionProxy->subscription = node->create_subscription<interfaces::msg::CartData>(in->topicName, qos, cb);
    }
