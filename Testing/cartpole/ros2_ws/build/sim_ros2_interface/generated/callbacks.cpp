...
void write__interfaces__msg__CartData(const interfaces::msg::CartData& msg, int stack, const ROS2WriteOptions *opt)
{
    try
    {
        sim::pushTableOntoStack(stack);
        try
        {
            ...
            write__float32(msg.velocity, stack, opt);
            sim::insertDataIntoStackTable(stack);
        }
        catch(std::exception &ex)
        {
        ...
        {
            // write field 'angle'
            sim::pushStringOntoStack(stack, "angle", 0);
            write__float32(msg.angle, stack, opt);
            sim::insertDataIntoStackTable(stack);
        }
        catch(std::exception &ex)
        {
            std::string msg = "field 'angle': ";
            msg += ex.what();
            throw sim::exception(msg);
        }
    }
    ...
    {
        std::string msg = "write__interfaces__msg__CartData: ";
        msg += ex.what();
        throw sim::exception(msg);
    }
}

void read__interfaces__msg__CartData(int stack, interfaces::msg::CartData *msg, const ROS2ReadOptions *opt)
{
    try
    {
        int r = sim::getStackTableInfo(stack, 0);
        ...
        while(numItems >= 1)
        {
            ...
            if((str = sim::getStackStringValue(stack, &strSz)) != NULL && strSz > 0)
            {
                ...
                else if(strcmp(str, "velocity") == 0)
                {
                    ...
                    {
                        // read field 'velocity'
                        read__float32(stack, &(msg->velocity), opt);
                    }
                    catch(std::exception &ex)
                    {
                        std::string msg = "field velocity: ";
                ...
                else if(strcmp(str, "angle") == 0)
                {
                    ...
                    {
                        // read field 'angle'
                        read__float32(stack, &(msg->angle), opt);
                    }
                    catch(std::exception &ex)
                    {
                        std::string msg = "field angle: ";
    ...
    {
        std::string msg = "read__interfaces__msg__CartData: ";
        msg += ex.what();
        throw sim::exception(msg);
    }
}
...
void ros_callback__interfaces__msg__CartData(const interfaces::msg::CartData::SharedPtr msg, SubscriptionProxy *proxy)
{
    int stack = -1;
    try
    {
        stack = sim::createStack();
        write__interfaces__msg__CartData(*msg, stack, &(proxy->wr_opt));
        sim::callScriptFunctionEx(proxy->topicCallback.scriptId, proxy->topicCallback.name.c_str(), stack);
        sim::releaseStack(stack);
        stack = -1;
    }
    catch(std::exception &ex)
    {
        if(stack != -1)
            simReleaseStack(stack); // don't throw
        std::string msg = "ros_callback__interfaces__msg__CartData: ";
        msg += ex.what();
        simSetLastError(proxy->topicCallback.name.c_str(), msg.c_str());
    }
}
